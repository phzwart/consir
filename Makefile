...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=consir tests/
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 consir tests
...
check-safety:
	...
	poetry run bandit -ll --recursive consir tests
