...
def sample_points_hierarchically(radius, domain=[(0, 1), (0, 1)], factor=np.sqrt(2.0)):
    ...
    """
    initial_sampler = hierarchical_poisson_disc.PoissonDiskSampler(
        r=radius, k=30, domain=domain
    )
    ...
    last_indices = np.arange(
        len(initial_set)
    )  # Initialize indices for all initial points
    ...
    # for k, dimso in enumerate(domain):
    #    min_val = dimso[0]
    #    max_val = dimso[1]
    #    all_points_array[:,k] = all_points_array[:, k]*(max_val - min_val) + min_val
