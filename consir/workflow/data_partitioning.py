...
def stratified_split(
    ...
    for class_label in unique_classes:
        ...
        if n_samples < (n_cal_desired + n_train_desired):
            print(f"Warning: Class {class_label} has only {n_samples} samples. "
            ...
            train_samples = rng.choice(class_indices, size=n_train_desired, replace=True)
    ...
    print(f"Using sampling with replacement: {len(set(train_idx)) + len(set(cal_idx)) < len(train_idx) + len(cal_idx)}")
...
def create_ensemble_splits(
    ...
    for split in range(n_splits):
        ...
        print(f"Unique training samples: {n_unique_train}/{len(train_idx)} "
              f"({100*n_unique_train/len(train_idx):.1f}%)")
        print(f"Unique test samples: {n_unique_test}/{len(test_idx)} "
