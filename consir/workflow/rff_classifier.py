...
class RFFClassifier(pl.LightningModule):
    ...
    def __init__(
        ...
        self.register_buffer(
            'frequencies',
            torch.randn(input_dim, n_features) / length_scale
        )
        
        # Initialize random offsets
        self.register_buffer(
            'offsets',
            2 * np.pi * torch.rand(n_features)
        )
    ...
    def training_step(
        ...
        # Log batch metrics
        self.log('train_loss', loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=len(x))
    ...
    def validation_step(
        ...
        # Log metrics
        self.log('val_loss', loss, prog_bar=True, on_step=False, on_epoch=True, batch_size=len(x))
        
        # Calculate and log accuracy
        preds = torch.argmax(log_probs, dim=1)
        acc = (preds == y).float().mean()
        self.log('val_acc', acc, prog_bar=True, on_step=False, on_epoch=True, batch_size=len(x))
